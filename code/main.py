...
def _init_():
    ...
    if not os.path.exists('/output/checkpoints/'+args.exp_name):
        os.makedirs('/output/checkpoints/'+args.exp_name)

    if not args.eval:  # backup the running files
        os.system('cp main.py /output/checkpoints' + '/' + args.exp_name + '/' + 'main.py.backup')
        os.system('cp util/PAConv_util.py /output/checkpoints' + '/' + args.exp_name + '/' + 'PAConv_util.py.backup')
        os.system('cp util/data_util.py /output/checkpoints' + '/' + args.exp_name + '/' + 'data_util.py.backup')
        if args.arch == 'dgcnn':
            os.system('cp model/DGCNN_PAConv.py /output/checkpoints' + '/' + args.exp_name + '/' + 'DGCNN_PAConv.py.backup')
        elif args.arch == 'pointnet':
            os.system('cp model/PointNet_PAConv.py /output/checkpoints' + '/' + args.exp_name + '/' + 'PointNet_PAConv.py.backup')

    global writer
    writer = SummaryWriter('/output/logs/' + args.exp_name)
...
def train(args, io):
    ...
    for epoch in range(args.epochs):
        ...
        if test_acc >= best_test_acc:
            ...
            torch.save(model.state_dict(), '/output/checkpoints/%s/best_model.t7' % args.exp_name)
        
        t1 = time.time()
        print("epoch[{}/{}] done, time cost {:.3f}\n".format(epoch+1, args.epochs, t1-t0))
...
def test(args, io):
    ...
    model.load_state_dict(torch.load("pointnet_best_model.t7"))
    model = model.eval()
    test_acc = 0.0
    count = 0.0
    test_true = []
...
if __name__ == "__main__":
    ...
    if not args.eval:
        io = IOStream('/output/checkpoints/' + args.exp_name + '/%s_train.log' % (args.exp_name))
    else:
        io = IOStream('/output/checkpoints/' + args.exp_name + '/%s_test.log' % (args.exp_name))
    io.cprint(str(args))
