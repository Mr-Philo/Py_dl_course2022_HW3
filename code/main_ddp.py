...
def get_logger():
    ...
    file_handler = logging.FileHandler(os.path.join('/output/checkpoints', args.exp_name, 'main-' + str(int(time.time())) + '.log'))
...
def train(gpu, ngpus_per_node):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if test_acc >= best_test_acc and main_process():
            ...
            torch.save(model.state_dict(), '/output/checkpoints/%s/best_model.t7' % args.exp_name)  # save the best model
...
def test(gpu, ngpus_per_node):
    ...
    state_dict = torch.load("/output/checkpoints/%s/best_model.t7" % args.exp_name, map_location=torch.device('cpu'))
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if main_process():
        ...
        if not args.eval:  # backup the running files
            os.system('cp main_ddp.py /output/checkpoints' + '/' + args.exp_name + '/' + 'main_ddp.py.backup')
            ...
            if args.arch == 'dgcnn':
                ...
                os.system(
                    'cp model/PointNet_PAConv.py /output/checkpoints' + '/' + args.exp_name + '/' + 'PointNet_PAConv.py.backup')
